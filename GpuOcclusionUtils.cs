...
namespace Examples.GpuOcclusion
{
    ...
    public class GpuOcclusionUtils
    {
        ...
        /// <summary>
        /// Obtener posicion 2D en pantalla
        /// </summary>
        /// <returns>False si esta fuera de la pantalla</returns>
        public static bool getScreenPos(Vector3 pos3d, out Vector2 pos2d)
        {
            pos2d = Vector2.Empty;
            ...
            Matrix proj = d3dDevice.Transform.Projection;
            Matrix m = view * proj;
            int width = d3dDevice.Viewport.Width;
            int height = d3dDevice.Viewport.Height;

            Vector4 pOut = Vector3.Transform(pos3d, m);
            if (pOut.W < 0) return false;

            Vector3 pOut2 = GpuOcclusionUtils.toScreenSpace(pOut, width, height);
            pos2d.X = pOut2.X;
            pos2d.Y = pOut2.Y;

            if (pos2d.X < 0 || pos2d.X >= pos2d.X + width) return false;
            if (pos2d.Y < 0 || pos2d.Y >= pos2d.X + height) return false;
